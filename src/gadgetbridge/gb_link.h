 ...
 *
 *   - gb_ble.cpp     Nordic UART Service over BLE (§1), the real thing.
 *   - gb_link_stdio.cpp  stdin/stdout, used by the native/SDL2 emulator build
 *                    so the UI and message handling can be exercised without a
 *                    board or a phone.
 *
 * Everything here is called from the Arduino loop() / LVGL context only. The
 * BLE implementation does its own hand-off from the NimBLE host task.
 */
#pragma once
