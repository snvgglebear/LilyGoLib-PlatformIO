 ...
 *
 * Layout is flex and percentage based, with the only per-device decision being
 * font size picked from the actual panel width, so the same source lays out on
 * the 502x410 Ultra, the 240x240 T-Watch-S3 and the emulator window.
 *
...
void notificationClicked(lv_event_t *event)
{
    ...
    trackBox(lv_msgbox_create(NULL), &s_detail_box);
    lv_obj_set_width(s_detail_box, LV_PCT(90));
...
void openThread(size_t index)
{
    ...
    s_thread_view = lv_obj_create(lv_layer_top());
    lv_obj_set_size(s_thread_view, LV_PCT(100), LV_PCT(100));
...
void maybeShowMessagePopup()
{
    ...
    closeBox(s_message_box);
    trackBox(lv_msgbox_create(NULL), &s_message_box);
    lv_obj_set_width(s_message_box, LV_PCT(90));
...
void refreshCall()
{
    ...
    trackBox(lv_msgbox_create(NULL), &s_call_box);
    lv_obj_set_width(s_call_box, LV_PCT(90));
...
void refreshFind()
{
    ...
    trackBox(lv_msgbox_create(NULL), &s_find_box);
...
void refreshFiredAlarm()
{
    ...
    trackBox(lv_msgbox_create(NULL), &s_alarm_box);
