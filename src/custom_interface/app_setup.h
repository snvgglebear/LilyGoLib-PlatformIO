...
/// and starts Gadgetbridge. Call once, after the display + LVGL are up
/// (beginLvglHelper() on hardware, the SDL window natively).
void setupGui();

/// Pumps LVGL, sleep state and Gadgetbridge. Call every iteration of
/// loop() (Arduino) or the native main loop, after ticking LVGL's clock.
void loopGui();
