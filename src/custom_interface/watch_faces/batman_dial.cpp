...
/// the APP_DIAL_ARC_SPACING px reserved beyond it.
...
static void refresh(lv_timer_t *t)
{
    ...
    set_hand(hand_hour, pts_hour, hour_deg, (radius * APP_DIAL_HOUR_HAND_PCT) / 100);
    set_hand(hand_min,  pts_min,  min_deg,  (radius * APP_DIAL_MIN_HAND_PCT) / 100);
    set_hand(hand_sec,  pts_sec,  sec_deg,  (radius * APP_DIAL_SEC_HAND_PCT) / 100);
...
static void build_face(lv_obj_t *screen)
{
    ...
    if (w <= 0 || h <= 0) {
        ...
        w = h = APP_DIAL_FALLBACK_SIZE;
    }
    centre_x = w / 2;
    centre_y = h / 2;

    // outer_r is the furthest anything may be drawn; the dial itself stops
    // APP_DIAL_ARC_SPACING short of it, leaving that ring for the battery
    // arc and its readout. Everything else on the face is derived from
    // `radius`, so widening APP_DIAL_ARC_SPACING shrinks the dial and its
    // contents together.
    int32_t outer_r = (w < h ? w : h) / 2 - APP_DIAL_FACE_MARGIN;
    radius = outer_r - APP_DIAL_ARC_SPACING;
    if (radius < APP_DIAL_MIN_RADIUS) {
        radius = APP_DIAL_MIN_RADIUS;
    }
    ...
    // way. Owning the numerals is what makes APP_DIAL_NUMERAL_GAP a real knob.
    ...
    lv_obj_set_style_border_width(dial, APP_DIAL_BORDER_WIDTH, 0);
    ...
    lv_obj_set_style_length(dial, APP_DIAL_SMALL_TICK_LENGTH, LV_PART_ITEMS);
    lv_obj_set_style_line_width(dial, APP_DIAL_SMALL_TICK_WIDTH, LV_PART_ITEMS);
    lv_obj_set_style_line_color(dial, SMALL_TICK_COLOR, LV_PART_ITEMS);
    lv_obj_set_style_length(dial, APP_DIAL_LARGE_TICK_LENGTH, LV_PART_INDICATOR);
    lv_obj_set_style_line_width(dial, APP_DIAL_LARGE_TICK_WIDTH, LV_PART_INDICATOR);
    ...
    // major tick's inner end is at radius - APP_DIAL_LARGE_TICK_LENGTH;
    // APP_DIAL_NUMERAL_GAP is
    ...
    const lv_font_t *numeral_font = APP_FONT_DIAL_NUMERAL;
    int32_t numeral_r = radius - APP_DIAL_LARGE_TICK_LENGTH - APP_DIAL_NUMERAL_GAP
    ...
    // `radius` is the dial radius, so the arc's outer edge lands
    // APP_DIAL_ARC_GAP + APP_DIAL_ARC_WIDTH beyond it -- within the
    // APP_DIAL_ARC_SPACING reserved above as long as APP_DIAL_ARC_SPACING
    // is at least APP_DIAL_ARC_GAP + APP_DIAL_ARC_WIDTH.
    int32_t arc_r = radius + APP_DIAL_ARC_GAP + APP_DIAL_ARC_WIDTH;
    ...
    lv_obj_t *batt_arc = make_arc(area, ARC_COLOR, APP_DIAL_ARC_WIDTH, arc_r,
    ...
    // inward by APP_DIAL_ARC_WIDTH, so the ring occupies
    // [arc_r - APP_DIAL_ARC_WIDTH, arc_r]), meaning anything past arc_r
    // clears it. Half the line height puts the label's inner edge -- not its
    // centre -- APP_DIAL_ARC_GAP px off the arc, and derives from the font so
    // changing it does not need a new magic number.
    const lv_font_t *readout_font = APP_FONT_DIAL_READOUT;
    ...
    int32_t label_r = arc_r + APP_DIAL_ARC_GAP + lv_font_get_line_height(readout_font) / 2;
    ...
    hand_hour = make_hand(area, HOUR_HAND_COLOR, APP_DIAL_HOUR_HAND_WIDTH);
    hand_min  = make_hand(area, MIN_HAND_COLOR, APP_DIAL_MIN_HAND_WIDTH);
    hand_sec  = make_hand(area, SEC_HAND_COLOR, APP_DIAL_SEC_HAND_WIDTH);
