 ...
 * a container from safe_area_place() or safe_area_rect() rather than
 * straight to lv_screen_active().
 *
 * Which one depends on the shape of what you are placing:
 *
 *   safe_area_place() - a horizontal band (a status bar, a row of buttons).
 *       Widest for short bands near the vertical middle, and it narrows
 *       sharply as the band reaches into a corner arc.
 *   safe_area_rect()  - anything full-height or full-screen (a tileview,
 *       a list, a whole app page). A full-height band is pinched to 170 px
 *       by its top row, so safe_area_place() is the wrong tool there;
 *       safe_area_rect() gives the 338x430 inscribed rect instead.
 *
...
/*A container spanning y..y+height, widened to exactly the width visible at
  ...
  safe_area_rect() - see the header comment above.
