...
bool gb_protocol_dispatch(const std::string &line, GbProtocolHandler &handler)
{
    ...
    if (t == "ver") {
        ...
        nav.dist_m = int_of(doc["dist_m"], -1);
        nav.eta = str_of(doc["eta"]);
        nav.dest = str_of(doc["dest"]);
        nav.pct = int_of(doc["pct"], -1);
        ...
        // §5.14: a partial update -- each field is independently optional, and
        // only the ones the phone actually sent are flagged in the struct.
        GbSettings settings;
        if (doc["notif_timeout_ms"].is<int32_t>()) {
            settings.has_notif_timeout_ms = true;
            settings.notif_timeout_ms = doc["notif_timeout_ms"].as<int32_t>();
        }
        if (doc["notif_vibrate"].is<bool>()) {
            settings.has_notif_vibrate = true;
            settings.notif_vibrate = doc["notif_vibrate"].as<bool>();
        }
        if (doc["pinned_mask"].is<int32_t>()) {
            settings.has_pinned_mask = true;
            settings.pinned_mask = static_cast<uint32_t>(doc["pinned_mask"].as<int32_t>());
        }
