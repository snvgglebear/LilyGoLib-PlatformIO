 ...
 * Each band is handed to safe_area_place() so nothing renders under the
...
void ui_home_build(lv_obj_t *parent)
{
    ...
    lv_obj_t *battery_area = safe_area_place(parent, 0, battery_h);
    if (battery_area) {
        ui_battery_status_create(battery_area);
    }

    lv_obj_t *pinned_area = safe_area_place(parent, battery_h, pinned_h);
    if (pinned_area) {
