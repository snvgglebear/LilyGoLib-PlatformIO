...
typedef struct {
    ...
    // Settings-sync fields (protocol §5.14 settings, watch-settings-sync-plan
    // §3/§4). clock_mode is stored as the underlying uint8_t of the ClockMode
    // enum in app_config.h, not the wire string -- the string is a wire-format
    // choice only. See app_config.h for the defaults/ranges.
    uint32_t notif_timeout_ms;      ///< notification toast auto-dismiss delay, ms
    uint8_t  notif_vibrate;         ///< non-zero to vibrate (GB_HAPTIC_TAP) on notification arrival
...
/**
 * @brief Load the persisted IR code list from NVS (or the emulator's seed list).
 *
 * On hardware, reads the "ir_codes" namespace blob; a missing, wrong-size or
 * wrong-version blob resets to an empty list and rewrites it. On the emulator
 * there is no NVS, so this seeds a small in-RAM demo list instead (see
 * plans/ir-remote-learn-mode-plan.md §3.4).
 *
 * @param list Populated with the loaded (or seeded) list.
 */
void hw_ir_codes_load(ir_code_list_t &list);

/**
 * @brief Persist the whole IR code list to NVS ("ir_codes" namespace).
 *
 * @param list The list to store.
 * @return true on success, false if it could not be persisted (e.g. no NVS on
 *         the emulator -- the caller keeps the in-RAM copy either way).
 */
